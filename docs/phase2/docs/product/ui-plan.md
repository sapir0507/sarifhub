...
Since Phase 2 the result is stored as an immutable snapshot when the scan completes ([ADR 0008](../adr/0008-gate-snapshot.md));
the Phase 1 mock still evaluates it live and is aligned in Phase 7.
...
CI endpoints (SARIF upload with API key, gate check) and the full contract are in [`docs/architecture/api.md`](../architecture/api.md).
